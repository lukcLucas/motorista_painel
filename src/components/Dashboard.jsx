...
import { AssignRun } from "@/components/AssignRun";
import { LayoutDashboard, Users, ClipboardList, UserPlus, History, PanelTopOpen, LogOut, Truck } from 'lucide-react';
...
export function Dashboard({ userRole, onLogout, logAction }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('callFinalized', refreshData);
    window.addEventListener('callReopened', refreshData);
    window.addEventListener('runAssigned', refreshData);
    ...
    return () => {
      ...
      window.removeEventListener('callFinalized', refreshData);
      window.removeEventListener('callReopened', refreshData);
      window.removeEventListener('runAssigned', refreshData);
    };
  ...
  const permissions = {
    master: {
      ...
      canViewCorridas: true, 
      canEditDrivers: true,
      canDeleteDrivers: true,
      canCallDrivers: true,
      canAssignRuns: true, 
      canFinalizeCalls: true,
      canReopenCalls: true,
    },
    adm: {
      canViewPainel: false, 
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <Tabs value={activeTab} className="w-full" onValueChange={setActiveTab}>
        <TabsList className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 mb-8">
          ...
          {currentPermissions.canViewCorridas && ( 
            <TabsTrigger 
              value="corridas" 
              className={`flex items-center justify-center gap-2 py-3 ${activeTab === "corridas" ? "tab-active" : ""}`}
            >
        ...
        {currentPermissions.canViewPainel && (
          <TabsContent value="painel" className="space-y-6">
            <MetricsPanel key={metricsKey} userRole={userRole} canReopenCalls={currentPermissions.canReopenCalls} logAction={logAction} />
            <WelcomeCard />
          </TabsContent>
        )}
        ...
        {currentPermissions.canViewCorridas && ( 
          <TabsContent value="corridas">
            <AssignRun
              setActiveTab={setActiveTab}
              userRole={userRole}
              canAssignRuns={currentPermissions.canAssignRuns}
              logAction={logAction}
            />
          </TabsContent>
        )}
        ...
        {currentPermissions.canViewPainelMotorista && (
          <TabsContent value="painel_motorista">
            <DriverCallPanel 
              userRole={userRole} 
              canFinalizeCalls={currentPermissions.canFinalizeCalls}
              canReopenCalls={currentPermissions.canReopenCalls}
              logAction={logAction}
            />
          </TabsContent>
        )}
